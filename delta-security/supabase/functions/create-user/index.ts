...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { username, password, full_name, role, hourly_rate, badge_number, grade_id, phone_number } = await req.json()
    ...
    const { error: profileError } = await admin
      .from('profiles')
      .update({ full_name, role, hourly_rate: finalHourlyRate, badge_number: badge_number || null, grade_id: finalGradeId, phone_number: phone_number || null })
