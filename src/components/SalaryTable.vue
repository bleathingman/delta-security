  ...
  <div>
    ...
    <div class="flex items-center gap-4 mb-6 flex-wrap">
      ...
      <div class="flex items-center gap-3">
        ...
        <span class="font-body text-sm tabular-nums" style="color: #D4AF37; min-width: 200px; text-align: center;">
          {{ weekLabel }}
        </span>
        ...
        <table class="w-full">
          <thead>
            <tr style="border-bottom: 1px solid #1a1a1a;">
              ...
              <th class="text-right px-5 py-4 text-xs tracking-[0.15em] uppercase font-body font-medium" style="color: #555;">Tranches 10min</th>
          ...
          <tbody>
            <tr v-for="(emp, i) in salaries" :key="emp.id"
              ...
              <td class="px-5 py-4">
                <span class="font-body text-xs tabular-nums" style="color: #666;">
                  {{ emp.badge_number || '—' }}
                </span>
              </td>
              <td class="px-5 py-4 text-right">
                <span class="font-body text-sm tabular-nums" style="color: #F5F5F0;">
                  {{ emp.total_services }}
                </span>
              </td>
              <td class="px-5 py-4 text-right">
                <span class="font-body text-sm tabular-nums" style="color: #888;">
                  {{ emp.total_minutes }}min
                </span>
              </td>
              <td class="px-5 py-4 text-right">
                <span class="font-body text-sm tabular-nums font-medium" style="color: #F5F5F0;">
                  × {{ emp.intervals_of_10 }}
                </span>
              </td>
              <td class="px-5 py-4 text-right">
                <span class="font-body text-sm tabular-nums" style="color: #666;">
                  {{ emp.hourly_rate }}$/h
                </span>
              </td>
              <td class="px-5 py-4 text-right">
    ...
    <div class="mt-4 p-4 rounded" style="background: rgba(212,175,55,0.04); border: 1px solid rgba(212,175,55,0.12);">
      <p class="text-xs font-body" style="color: #666;">
        ...
        Salaire = ⌊ Minutes totales ÷ 10 ⌋ × (Taux horaire ÷ 6)
        &nbsp;·&nbsp; Exemple : 90min × 15$/h → 9 tranches × 2.50$ = <strong style="color: #D4AF37;">22.50$</strong>
      </p>
    </div>
  </div>
</template>
...
const { allServices, allProfiles } = storeToRefs(servicesStore)
